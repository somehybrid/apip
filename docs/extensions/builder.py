...
from sphinx.builders.gettext import (
...
class DPYMessageCatalogBuilder(MessageCatalogBuilder):
    ...
    def finish(self) -> None:
        ...
        context = {
            ...
            "ctime": datetime.datetime.fromtimestamp(timestamp, ltz).strftime(
                "%Y-%m-%d %H:%M%z"
            ),
        ...
        for textdomain, catalog in status_iterator(
            ...
            content = GettextRenderer(
                template_path="_templates/gettext", outdir=self.outdir
            ).render("message.pot_t", context)
